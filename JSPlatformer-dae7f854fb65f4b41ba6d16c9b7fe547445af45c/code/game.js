...
Level.prototype.obstacleAt = function(pos, size){
  var xStart = Math.floor(pos.x); //always rounds a decimal down; takes leftmost bounds of x
  var xEnd = Math.ceil(pos.x + size.x); //opposite of floor, always rounds up; takes rightmost bounds of x
  var yStart = Math.floor(pos.y);
  var yEnd = Math.ceil(pos.y +size.y);

  //consider sides/top/bottom as walls
  if(xStart < 0 || xEnd > this.width || yStart < 0 || yEnd > this.height)
    return "wall";

  //check grid position starting at yStart, xStart
  ...
  for(var y=yStart; y<yEnd; y++){
    ...
    for(var x=xStart; x<xEnd; x++){
      ...
      if(fieldType) //if it's anything other than null
        return fieldType;
    }
  }
}
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  var obstacle = level.obstacleAt(newPos, this.size); //will return null or obstacle type
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  //the floor is also an obstacle -- only allow jump if
